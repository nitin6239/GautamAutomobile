...
const Icons = {
  ArrowLeft: () => <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>,
  ArrowRight: () => <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m9 18 6-6-6-6"/></svg>,
  ...
  Speed: () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 14a2 2 0 1 0 0-4 2 2 0 0 0 0 4Z"/><path d="M12 2v2"/><path d="M12 20v2"/><path d="m4.93 4.93 1.41 1.41"/><path d="m17.66 17.66 1.41 1.41"/><path d="M2 12h2"/><path d="M20 12h2"/><path d="m6.34 17.66-1.41 1.41"/><path d="m19.07 4.93-1.41 1.41"/></svg>,
  Fuel: () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 22v-8a2 2 0 0 1 2-2h2.5"/><path d="M13 22v-8a2 2 0 0 0-2-2H8.5"/><path d="M11 2h2"/><path d="M12 2v4"/><path d="M8 6h8"/><path d="M19 22v-8a2 2 0 0 0-2-2h-2.5"/><path d="M5 6V4a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v2"/></svg>,
  Transmission: () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>,
...
const PremiumCarDetail = () => {
  ...
  useEffect(() => {
    ...
    const foundCar = CAR_DATA.find(c => {
      ...
      const slug = c.slug || c.name.replace(/\s+/g, '-').toLowerCase();
      return stringId === id || slug === id || c.name.replace(/\s+/g, '-').toLowerCase() === id;
    });
    ...
    if (foundCar) {
      ...
      console.error("Car not found for slug:", id);
    }
  }, [id]);

  if (!car) {
    return (
      <div className={`premium-container ${theme === 'light' ? 'light' : ''}`} style={{display:'flex', alignItems:'center', justifyContent:'center', height:'100vh'}}>
        <div style={{color: theme === 'light' ? '#333' : 'white', fontSize: '1.2rem'}}>Loading Luxury Experience...</div>
      </div>
    );
  }
  ...
  const currentImages = activeTab === 'exterior' 
    ? (exteriorImages.length > 0 ? exteriorImages : []) 
    : (interiorImages.length > 0 ? interiorImages : exteriorImages);
  ...
  return (
    ...
    <div className={`premium-container ${theme === 'light' ? 'light' : ''}`}>
        ...
        <div className="gallery-section">
          <div className="gallery-tabs">
            ...
            <button 
              ...
              onClick={() => { setActiveTab('interior'); setCurrentImgIndex(0); }}
              disabled={interiorImages.length === 0}
              style={{ opacity: interiorImages.length === 0 ? 0.5 : 1, cursor: interiorImages.length === 0 ? 'not-allowed' : 'pointer' }}
            >
              Interior View {interiorImages.length === 0 && '(No Images)'}
            </button>
          </div>
